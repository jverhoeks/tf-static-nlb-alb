...
ALB_LISTENER = os.environ['ALB_LISTENER']
...
PENDING_DEREGISTRATION_FILENAME = 'Pending deregisteration IP list of {}.json'.format(
    ALB_DNS_NAME)
...
def target_group_list(ip_list, port):
    """
...
def handler(event, context):
    ...
    alb_ports = ALB_LISTENER.split(",")
    nlb_tg_args = NLB_TG_ARN.split(",")
        #
    if len(alb_ports) != len(nlb_tg_args):
        print("ERROR: ALB_LISTENER AND NLB_TG_ARN have different length")
        sys.exit(1)
    ...
    while i <= MAX_LOOKUP_PER_INVOCATION:
        dns_lookup_result = dns_lookup(
            ALB_DNS_NAME, "A", authoritative_server_ip_list)
    ...
    # construct set of new active IPs and registered IPs
        ...
        for index in range(len(nlb_tg_args)):
            registered_ip_list = describe_target_health(nlb_tg_args[index])
            ...
            old_active_ip_dict = json.loads(
                download_ip_list(S3_BUCKET, ACTIVE_IP_LIST_KEY))
            old_pending_ip_dict = json.loads(
                download_ip_list(S3_BUCKET, PENDING_IP_LIST_KEY))
            ...
            if old_active_ip_dict:
                ...
                registration_ip_list = list(
                    new_diff_ip_set_from_s3 | new_diff_ip_set_from_descibe)
            ...
            if old_active_ip_dict:
                ...
                if old_pending_ip_dict:
                    ...
                    print("INFO: Missing pending IPs (pending " \
                          "IPs in the last but not the current invocation) - {}".format(
                              missing_ip_set))
            ...
            upload_ip_list(S3_BUCKET, ACTIVE_FILENAME,
                           active_ip_json, ACTIVE_IP_LIST_KEY)
            upload_ip_list(S3_BUCKET, PENDING_DEREGISTRATION_FILENAME,
                           pending_ip_json, PENDING_IP_LIST_KEY)

            if registration_ip_list:
                registerTarget_list = target_group_list(
                    registration_ip_list, int(alb_ports[index]))
                register_target(nlb_tg_args[index], registerTarget_list)
            ...
            if new_pending_ip_dict:
                ...
                for ip in pending_ip_list:
                    if new_pending_ip_dict[ip] >= INVOCATIONS_BEFORE_DEREGISTRATION:
                        ...
                        deregisterTarget_list = target_group_list(
                            deregistration_ip_list, int(alb_ports[index]))
                        deregister_target(
                            nlb_tg_args[index], deregisterTarget_list)
            else:
